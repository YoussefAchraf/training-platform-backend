...
# ---- deps: full install (incl. devDependencies), used to build and to run migrations ----
FROM node:22-alpine AS deps
WORKDIR /app
# bcrypt is a native module; build tools are only needed transiently to compile it
# when no prebuilt binary matches this platform/libc. Removed again in this same
# layer so they never inflate the (already-discarded) build-stage image.
...
# ---- prod-deps: production-only install, kept separate from the build's devDependencies ----
