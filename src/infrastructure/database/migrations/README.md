...
| 0007 | normalize_announcement_target_roles | 1NF fix (expand step). Read-only preflight, then the junction table `feature_announcement_target_roles` (FKs to announcements and roles, index on role), back-filled from the JSON list without modifying any row, a `CHECK` that the JSON is an array (NOT VALID) and a sync trigger so releases that still write only the JSON column keep the table correct. The JSON column stays until an approved contract migration. |
| 0008 | validate_announcement_target_roles_check | Validates 0007's check in its own transaction. |
| 0009 | document_schema | `COMMENT ON` for every table and the non-obvious columns, so the design is visible in pgAdmin and feeds the generated docs. Metadata only. |
