...
describe('UploadChunkUseCase', () => {
  ...
  it('rejects a non-Buffer body instead of treating it as a valid chunk (express.raw() defaults req.body to {} for an empty request)', async () => {
    const deps = buildDeps();
    const useCase = new UploadChunkUseCase(deps);

    await expect(
      useCase.execute({ requester: buildRequester(), uploadId: 'u1', index: 0, buffer: {} as any })
    ).rejects.toThrow('Empty chunk');
    await expect(
      useCase.execute({ requester: buildRequester(), uploadId: 'u1', index: 0, buffer: 'not a buffer' as any })
    ).rejects.toThrow('Empty chunk');
    expect(deps.attachmentStorageService.writeChunk).not.toHaveBeenCalled();
  });
